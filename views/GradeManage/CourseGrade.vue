    ...
    <div>
        ...
        <el-card>
            <el-table :data="gradeList" stripe style="width: 100%">
                ...
                <el-table-column label="查看实验成绩" width="250">
                    <template slot-scope="scope">
                        <!-- <el-input v-model="labGradeSearchModel.classId" placeholder="课程id" clearable></el-input>
                            <el-input v-model="labGradeSearchModel.userId" placeholder="学生id" clearable></el-input> -->
                        <el-button @click="getLabGradeList(scope.row)" type="primary" icon="el-icon-search"
                            round>实验成绩</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </el-card>
        ...
        <!-- 结果列表 -->
        <div v-show="isShowLabGrade" style="position: absolute;left: 50%;right: 0%;  top: 30%;
        bottom: 0;width: 800px;;margin: 0px auto;">
            <el-card>
                <el-table :data="labGradeList" stripe style="width: 100%">
                    <el-table-column label="#" width="50">
                        <!-- scope为当前元素 -->
                        <template slot-scope="scope">
                            <!-- (pageNo - 1) * pageSize + index + 1 -->
                            {{ (labGradeSearchModel.pageNo - 1) * labGradeSearchModel.pageSize + scope.$index + 1 }}
                        </template>
                    </el-table-column>
                    <el-table-column prop="labId" label="实验ID" width="100">
                    </el-table-column>
                    <el-table-column prop="LabName" label="实验名称" width="100">
                    </el-table-column>
                    <el-table-column prop="userId" label="学工号" width="100">
                    </el-table-column>
                    <el-table-column prop="labGrade" label="实验成绩" width="200">
                    </el-table-column>
                </el-table>
                <!-- 分页组件 -->
                <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                    :current-page="labGradeSearchModel.pageNo" :page-sizes="[5, 10, 20, 40]"
                    :page-size="labGradeSearchModel.pageSize" layout="total, sizes, prev, pager, next, jumper"
                    :total="total">
                </el-pagination>
            </el-card>
        </div>
